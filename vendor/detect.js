  ...
  */
  ...
  VersionManager.prototype.getVersions = function() {
    ...
    for (var i = 0; i < links.length; i++) {
      ...
      var os = links[i].getAttribute('os');
      var browser = links[i].getAttribute('browser');
      var type = links[i].getAttribute('type');
      var id = links[i].getAttribute('id');
      versions.push(new Version(href, media, os, browser, type, id));
    }
  ...
  function Version(href, mediaQuery, os, browser, type, id) {
  ...
  Version.prototype.matches = function() {
    ...
    if (this.mediaQuery) {
      ...
      var osParser = new UAParser();
      return osParser.evaluateOS(this.os);
    } else if (this.browser) {
      var browserParser = new UAParser();
      return browserParser.evaluateBrowser(this.browser);
    } else if (this.type) {
      var typeParser = new UAParser();
      return typeParser.evaluateType(this.type);
    }
  };
  ...
  /**
   * Evaluates the os with user-agent.
   */
  UAParser.prototype.evaluateOS = function(os) {
    const currentOS = sanitize(this.getOS().name)
    const redirectOS = sanitize(os)
    return matchCriteria(currentOS, redirectOS)
  }

  /**
   * Evaluates the browser with user-agent.
   */
  UAParser.prototype.evaluateBrowser = function(browser) {
    const currentBrowser = sanitize(this.getBrowser().name)
    const redirectBrowser = sanitize(browser)
    return matchCriteria(currentBrowser, redirectBrowser)
  }
  ...
  UAParser.prototype.evaluateType = function(type) {
    ...
    if(!device) {
      console.error('No matched device type')
      return;
    }
    const currentType = sanitize(this.getDevice().type)
    const redirectType = sanitize(type)
    return matchCriteria(currentType, redirectType)
  }
  ...
  function matchCriteria (user, redirect) {
    return user.includes(redirect)
  }
